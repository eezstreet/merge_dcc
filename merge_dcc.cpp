...
void my_credits(void)
{
   printf("MERGE_DCC Freeware v. 1.8, by Paul SIRAMY, 24 May 2007\n"
		  "Special Edition for Rapture by Nick Whitlock, 9 September 2014\n"
...
int main (void)
{
   ...
   INILoader* INI = new INILoader(txtname);
   if(!INI->Loaded()) {
	   cerr << INI->GetErrorMessage() << "\n";
	   if(cofmem != NULL) {
		   free(cofmem);
	   }
	   delete INI;
   ...
   for (i=0; i<COMP_MAX; i++)
   {
	  if (cof_comp[i].present && cof_comp[i].active)
	  {
		 ...
		 else
		 {
			...
			cout << "* " << comp_str[i] << " : " << cof_comp[i].filepath
				<< " (" << (cof_comp[i].cmapname[0] ? cof_comp[i].cmapname : "none") << ", block " << cof_comp[i].palette << ")\n";
		 }
	  }
   }
   if (ok)
   {
	  for (i=0; i<COMP_MAX; i++)
	  {
		 if (strlen(cof_comp[i].filepath))
		 {
			...
			if (dcc[i] == NULL)
			{
				cerr << "Fatal error while loading " << cof_comp[i].filepath << " :\n" << dcc_error;
			}
			else
			{
			   if (dcc_decode(dcc[i], -1))
			   {
				   cerr << "Fatal error while decoding " << cof_comp[i].filepath << " :\n" << dcc_error;
			   }
			   else
